...
def return_output_analysis(true, pred):
    ...
    output_file = "results/classification_scores.txt"
...
def plot_confusion_matrix(true, pred):
    ...
    text_color = np.array(['black'] * len(conf_melt))  # change color according to box color
    ...
    for i in range(0, len(conf_melt)):
        if conf_melt["Prediction"][i] < 0.005:
            conf_melt["Prediction"][i] = 0
        else:
            conf_melt["Prediction"][i] = np.round(conf_melt["Prediction"][i], decimals=2)

    plot = (p9.ggplot(conf_melt, p9.aes('Predicted Class', 'rna_type', fill="Prediction"))
            + p9.geom_tile(p9.aes(width=.95, height=.95))
            + p9.geom_text(p9.aes(label='pred_string'), size=8, color=text_color, fontweight="bold")
            + p9.scale_y_discrete(limits=list(reversed(rna_types)))
            + p9.scale_fill_distiller(type="seq", palette="OrRd", direction=1, guide=False)
    ...
    plot.save(filename="results/confusion_matrix.png", dpi=300)
